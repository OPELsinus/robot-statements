...
class Builder:
    ...
    def upd_metadata(self, major=False, minor=False, micro=False):
        ...
        minor_ = version[1] + 1 if minor else version[1]
        micro_ = version[2] + 1 if micro else version[2]
...
if __name__ == '__main__':
    ...
    command_ = ['pyinstaller.exe', '-F', '-c', '-a', '--clean', '-n', builder.metadata.original_filename, 'main.py',
                '--specpath', f'{builder.build_dir}', '--version-file', f'{builder.version_file.name}',
                '-i', f'app.ico', '--add-data', f'{root_path.joinpath("app.ico").__str__()};.']
